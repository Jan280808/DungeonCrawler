...
public class Cube {
    ...
    public void loadBlockMap(Material material) {
        AtomicInteger i = new AtomicInteger();
        blockList().forEach(block -> {
            if(block.getType() == material) {
                if(block.getLocation().add(0, 1, 0).getBlock().getType().equals(Material.AIR)) {
                    i.set(i.get() + 1);
                    blockMap.put(i.get(), block);
                }
            }
        });
    }

    //return random block != type air
    public Block getRandomBlockFromMap() {
        return blockMap.get(new Random().nextInt((blockMap.size() - 1) + 1) + 1);
    }
}
